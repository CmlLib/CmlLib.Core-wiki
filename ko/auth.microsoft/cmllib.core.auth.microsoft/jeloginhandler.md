...
이 메서드는 먼저 [#가장 최근 플레이한 계정으로 로그인](jeloginhandler.md#가장 최근 플레이한 계정으로 로그인 "mention")을 시도하고, 만약 실패한다면 [#새로운 계정으로 로그인](jeloginhandler.md#새로운 계정으로 로그인 "mention")을 시도합니다.
